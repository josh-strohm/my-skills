...
Authorization: Bearer <access_token from Step B-2>
Content-Type: application/json
...
Once you have the key, every request to transcriptapi.com needs two headers: `Authorization: Bearer $TRANSCRIPT_API_KEY` and a `User-Agent` identifying your agent. Omitting User-Agent causes Cloudflare to return a 403 (error code 1010). Use your agent's name and version — agent name alone is fine if version is not readily available.
